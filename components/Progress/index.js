  ...
  render() {
    ...
    if (props.showInfo) {
      if (props.status === 'exception') {
        ...
        progressInfo = (
          <span className={`${prefixCls}-line-text`} style={{fontSize: this.props.fontSize}}>{text}</span>
        );
      }
    } else {
...
let Circle = React.createClass({
  ...
  statusColorMap() {
    return {
      normal: this.props.strokeColor || '#2db7f5',
      exception: '#ff5500',
      success: '#87d068'
    }
  },
  ...
  render() {
    ...
    if (parseInt(props.percent, 10) === 100 && !props.noSuccess) {
      props.status = 'success';
    }
    ...
    if (props.status === 'exception') {
      ...
      progressInfo = (
        <div className={`${prefixCls}-circle-text`}>
      ...
      <div className={`${prefixCls}-circle-wrap status-${props.status}`} style={props.style}>
        <div className={`${prefixCls}-circle-inner`} style={style}>
          <Progresscircle percent={props.percent} strokeWidth={props.strokeWidth}
            strokeColor={this.statusColorMap()[props.status]} trailColor={props.trailColor} />
          {progressInfo}
        </div>
      </div>
    );
